...
export async function verify_user_data_token(token: string): Promise<[true, true, Users] | [false, undefined | APIResultType, undefined]> {
    ...
    const cached_user_data = JSON.parse(await redis.get(`cached-user-data-${result}`) || "{}");
    ...
    if(parsed_cached_user_data.success) {
        ...
        try {
            ...
            await redis.setEx(`cached-user-data-${result}`, 60*60*24*2, JSON.stringify(user_data));
        }
        catch (err) {user_data
