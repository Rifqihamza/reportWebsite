...
export default function OtherOptionsForm() {
  ...
  return (
    <>
      ...
      <InputField
        label="Gambar berupa link"
        description="Jika aktif, kolom gambar akan diisi link menuju gambar yang sesuai dan tidak menampilkan gambar nya langsung"
        inputType="toggle"
        onChange={(value) => setOtherOption("usingLinkInsteadOfImage", value)}
        value={otherOption.usingLinkInsteadOfImage}
        disabled={selectedOutputType !== ExportOutputType.PDF}
      />
    </>
  );
}
        ...
        if (props.inputType == "text") {
          ...
          return (
            <div className="relative">
              <input
                className={`p-2 w-24 bg-[#14161b] text-white border ${props.max && props.value > props.max ? "border-red-500" : "border-transparent"}`}
                type="number"
                value={props.value}
          ...
          return (
            <div
              className={`min-w-18 h-11 relative bg-[#13161b] border duration-500 ${props.value ? "border-white opacity-100" : "border-[#374151] opacity-75"} rounded-full ${props.disabled ? "opacity-25! cursor-not-allowed" : ""}`}
            >
              <input
                className="absolute top-0 left-0 w-full h-full opacity-0 z-100"
                type="checkbox"
                checked={props.value}
              ...
              ></input>
              <div
                className={`absolute top-[4px] duration-500 left-[4px] ${props.value ? "left-[calc(100%_-_4px)] -translate-x-full" : ""} h-[calc(100%_-_8px)] aspect-square rounded-full bg-white`}
              ></div>
            </div>
          );
        } else {
