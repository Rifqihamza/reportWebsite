...
import { AccountAPIPrivillage, type User } from "../../../types/variables";
...
export default function UsersPage() {
  ...
  const { showedUserAccountData, setUserAccountData, doneFetching, userAccountData, isAuthorized: isAuhtorizedGetAllUsers } = useUserAccountHook();
  const { showMessage } = useMessageToastHook();
  const { isConnected } = useNetworkConnectivityHook();
  const { userData, userPrivillages: userDataPrivillages } = useUserDataHook();

  const { setChangingUserData: setEditedUser, setVisibleDialog, visibleDialog } = useEditUserDataHook();
  ...
  return (
    <>
      ...
      {(() => {
        ...
        if (!doneFetching) {
          return (
            <div className="w-full h-full flex justify-center items-center z-100 relative bg-transparent rounded-xl">
        ...
        if (!isAuhtorizedGetAllUsers) {
          return (
            <div className="text-white w-full h-full flex justify-center items-center z-100 bg-transparent rounded-xl">
              <p>Maaf, anda tidak mendapat akses untuk melihat data pengguna.</p>
            </div>
          );
        }
                    ...
                    <tr key={user.id} className="border-b border-[#F97316] text-white font-semibold">
                      ...
                      <td className="flex flex-row items-center justify-center gap-2 p-4 h-full">
                        <button
                          ...
                          className="cursor-pointer bg-blue-400 text-white px-4 py-1 rounded-xl duration-200 hover:brightness-75 disabled:brightness-75 disabled:opacity-50 disabled:cursor-not-allowed"
                          disabled={!userDataPrivillages.includes(AccountAPIPrivillage.UpdateUser)}
                        >
                          Edit
                        </button>
                        <span>|</span>
                        <button
                          className="cursor-pointer bg-red-400 text-white px-4 py-1 rounded-xl duration-200 hover:brightness-75 disabled:brightness-75 disabled:opacity-50 disabled:cursor-not-allowed"
                          onClick={() => handleDelete(user)}
                          disabled={!userDataPrivillages.includes(AccountAPIPrivillage.DeleteUser) || deletedUserIDProcess ? true : false}
                        >
                          Delete
                        </button>
                      </td>
              ...
              {showedUserAccountData.map((user, index) => (
                <div key={user.id} className="flex flex-col p-6 gap-2 bg-[#374151] w-full rounded-2xl">
                  ...
                  <div className="flex flex-row items-center justify-center py-2 gap-2 h-full">
                    <button
                      onClick={() => handleEditClick(user)}
                      className="bg-blue-400 text-white px-4 py-1 rounded-xl w-full disabled:brightness-75 disabled:opacity-50 disabled:cursor-not-allowed"
                      disabled={!userDataPrivillages.includes(AccountAPIPrivillage.UpdateUser)}
                    >
                      Edit
                    </button>
                    <button
                      className="bg-red-400 text-white px-4 py-1 rounded-xl w-full disabled:brightness-75 disabled:opacity-50 disabled:cursor-not-allowed"
                      onClick={() => handleDelete(user)}
                      disabled={!userDataPrivillages.includes(AccountAPIPrivillage.DeleteUser) || deletedUserIDProcess ? true : false}
                    >
                      Delete
