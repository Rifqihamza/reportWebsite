  ...
  return (
    <PrimeReactProvider>
      <Dialog
        ...
        header={
          <div>
            <h1 className="text-md">Upload Bukti Foto</h1>
            <p className="text-sm font-thin">Pilih file gambar sebagai bukti laporan telah selesai</p>
          </div>
        }
      >
        <div className="flex items-center justify-center w-full z-10">
          ...
          >
            <div className="flex flex-col items-center justify-center">
              ...
              <p className={`mb-1 text-sm`} id="file-name-display">
                {image ? image.name : "Klik untuk upload foto"}
              </p>
              <p className={`text-xs text-${image ? "black" : "black"}`}>
                {image
                  ? `${image.type} (${image.size.toString().length > 6 ? Math.round(image.size / 10000) / 100 + "MB" : Math.round(image.size / 10) / 100 + "KB"})`
                  : `PNG, JPG atau JPEG (Max. ${max_file_size}MB)`}
              </p>
            </div>
            ...
            <input
              ...
              onChange={(e) => {
                e.target.files
                  ? e.target.files[0].size < max_file_size * 1000000
                    ? setImage(e.target.files[0])
                    : showMessage("Image is too large!", "warn", `Please put an image smaller than ${max_file_size}MB`)
                  : "";
              }}
            />
          </label>
