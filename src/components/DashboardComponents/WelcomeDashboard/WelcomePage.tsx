...
function QuickNavigationButton(props: { icon: string; title: string; description: string; onClick: () => void }) {
  return (
    <button className="cursor-pointer w-fit md:h-18 h-13 aspect-square p-2 rounded-xl bg-[#F97316] hover:bg-[#C86B3F] text-white font-semibold duration-300" onClick={props.onClick}>
...
function DataReport(props: { title: string; value: number; icon: string; status: string }) {
  ...
  return (
    <div className="w-full md:w-1/2 h-fit p-4 rounded-xl flex flex-col gap-2 text-white" style={{ backgroundColor: bgColor }}>
      ...
      <p className="text-2xl md:text-4xl whitespace-nowrap md:whitespace-normal">
        {typeof props.value === "number" && !isNaN(props.value) ? props.value : <i className="pi pi-spinner pi-spin" style={{ fontSize: 18 }} />} <span className="text-lg">Laporan</span>
      </p>
    </div>
  );
}
...
export default function WelcomePage() {
  const { setActiveTab, activeTab } = useDashboardSidebarHook();
  const { userData } = useUserDataHook();
  const { reportData } = useReportDataHook();
  UseReportDataHookEffect();

  const currentHour = new Date().getHours();
  const greeting = currentHour > 18 || currentHour < 5 ? "Selamat Malam 🌙" : currentHour > 12 ? (currentHour >= 15 ? "Selamat Sore 🌤️" : "Selamat Siang ☀️") : "Selamat Pagi 🌞";
  ...
  return (
    <section className="px-5 py-10 md:py-6 overflow-auto space-y-4">
      ...
      {/* Header */}
      <div className="w-full h-fit bg-[#374151] flex flex-col md:flex-row items-center justify-between gap-4 px-3 py-4 md:px-6 md:py-8 rounded-xl">
        <header>
          <div className="text-white">
            <h1 className="text-md">
              {greeting}, <span className="font-semibold">{userData?.username || "User"}!</span>
            </h1>
