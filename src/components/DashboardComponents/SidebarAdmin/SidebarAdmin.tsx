...
export default function SidebarDashboard() {
  const { showSidebar, setShowSidebar, activeTab, setActiveTab } = useDashboardSidebarHook();
  const { userData, userPrivillages } = useUserDataHook();
  const { isConnected } = useNetworkConnectivityHook();

  const [openedMenuTab, setOpenedMenuTab] = useState<{ id: number; isOpen: boolean }[]>(
    menuItems
      .map((item, index) => ({ id: index, isFiltered: (item as any).items !== undefined }))
      .filter((item) => item.isFiltered)
      .map((item) => ({ id: item.id, isOpen: false })),
  );
  ...
  const renderMenuItem = (menu: MenuItem | MenuItemGroup, index: number) => {
    if ((menu as any).items) {
        ...
        <li key={index}>
          <div className="">
            <button
              ...
              className={`relative w-full flex flex-row items-center text-left group py-3 font-semibold uppercase tracking-wider space-x-3 transition-colors duration-300 ${
            ...
            >
              ...
              <span className={`absolute bottom-0 left-0 h-1 bg-white rounded-full transition-all duration-500 ${isGroupOpened ? "w-full" : "w-0 group-hover:w-full group-hover:left-0"}`}></span>
            </button>
              ...
              {menu.items.map((item, index) => {
                return (
                  <button
                    ...
                    className={`p-3 relative w-full text-left group py-3 font-semibold uppercase tracking-wider space-x-3 transition-colors duration-300 ${
                      activeTab === item.id ? "text-white bg-black/50" : "text-gray-300"
                    }`}
                  >
                    {item.label}
                  </button>
      ...
      // TODO Debug mode, ganti dulu woi
      ...
      return (
        <li key={index}>
          ...
          >
            <i className={menu.icon}></i>
            {menu.label}
            <span className={`absolute bottom-0 left-0 h-1 bg-white rounded-full transition-all duration-500 ${activeTab === menu.id ? "w-full" : "w-0 group-hover:w-full group-hover:left-0"}`}></span>
          </button>
        </li>
      );
    }
    ...
    >
      <div className="flex flex-col gap-2 h-full px-4 transform transition-all duration-300 ease-in-out whitespace-nowrap opacity-100 translate-x-0">
        <ul className="relative h-full space-y-2">
          ...
          <li>
            <button onClick={handleLogout} className={`relative w-full text-left group py-3 font-semibold uppercase tracking-wider space-x-3 transition-colors duration-300 text-[#F2F0EF]`}>
